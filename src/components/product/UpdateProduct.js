...
import {detailsProductAction, updateproduct} from "../../features/actions/productAction"
import { NEW_PRODUCT_RESET, UPDATE_PRODUCT_RESET } from '../../features/constants/ProductConstants';
...
export default function UpdateProduct() {
    ...
    const {loading,detailsProduct} = useSelector(state=>state.detailsProduct)
...
const createProductSubmitHandler = (e) => {
  ...
  dispatch(updateproduct(id,{
    ...
    genre:genre,
    benefice:price-providerPrice,
    height:localStorage.getItem("HEIGHT_APACHER")? JSON.parse(localStorage.getItem("HEIGHT_APACHER")):height,
    colors:localStorage.getItem("COLORS_APACHER")? JSON.parse(localStorage.getItem("COLORS_APACHER")):colors,
  }));
...
useEffect(() => {
  fetchProduct()
  localStorage.removeItem("HEIGHT_APACHER")
  localStorage.removeItem("COLORS_APACHER")
  if (error) {
    alert.error(error);
    dispatch(clearErrors());
      ...
      fetchProduct()
      if(localStorage.getItem("UPDATE_PRODUCT_APACHER"))
      {
        let detailsP = JSON.parse(localStorage.getItem("UPDATE_PRODUCT_APACHER"))

        nameRef.current.value = "cool"
        setProvider(detailsP?.provider)
        setImages(detailsP?.images)
        setTitle(detailsP?.title)
        setDescription(detailsP?.description)
        setProviderPrice(detailsP?.providerPrice)
        setPrice(detailsP?.price)
        setCategory(detailsP?.category)
        setBrand(detailsP?.brand)
        setColors(detailsP?.colors)
        const b = detailsP?.price - detailsP?.providerPrice
        setBenenfice(b)
        setHeight(detailsP?.height)
        // height = [...detailsProduct?.height]
        setGenre(detailsP?.genre)
        setRunLoading(true)
        setTimeout(
            ()=>{
                setRunLoading(false)
...
<>
    <div className='px-7 py-5 h-full'>
        <form onSubmit = {createProductSubmitHandler} className='h-full p-5 lg:p-10 rounded-md flex flex-row justify-start items-left' style = {{backgroundColor:"#f3f3f3"}}>
          <div className ="flex flex-col justify-start">  
            <h1 className ="font-bold mb-2">Modifier un produit</h1>
            ...
            <div className='mt-7 flex flex-col lg:flex-row items-center justify-start gap-5'>
                <TextField ref={nameRef} value = {title} onChange={(e)=>setTitle(e.target.value)} className='w-500' id="outlined-basic" label="Nom produit" variant="outlined" />
          ...
          <div className='mt-10 flex flex-col items-center justify-center '>
             <label className ="cursor-pointer">
                 ...
                 <div className ="relative">
                     ...
                     {newImages?.length >0? newImages.length:images?.length}
                     </div>
  ...
  <div className='px-7 py-5 h-full'>
        <p className='font-bold  mb-5'>Images (  {newImages?.length >0? newImages.length:images?.length})</p>
        <div className ="p-5 lg:p-10 gap-3 flex justify-start items-center" style ={{backgroundColor:"#f3f3f3"}}>
          {
               newImages.length>0 ?  newImages?.map(
                ...
                (item)=>{
                    return (
                      <div >
                          <img className = "rounded-md" style = {{width:"205px",height:"310px"}} src = {item?.url} />
                      </div>
                    )
                }
            )
