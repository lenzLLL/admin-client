...
import {NEW_PC_RESET} from '../../features/constants/productCategoryConstant';
import {clearErrors} from "../../features/actions/productAction"
import {createPc} from "../../features/actions/producCategoryAction"
export default function NewProductCategory() {

    const dispatch = useDispatch()
    const alert = useAlert()
    const [show,setShow] = useState(false)
    const {loading,error,success} = useSelector(state => state.newPc)
    ...
    <div className='px-7 py-5 h-full'>
        <form onSubmit = {createProductSubmitHandler} className='h-full p-5 lg:p-10 rounded-md flex flex-row justify-start items-left' style = {{backgroundColor:"#f3f3f3"}}>
          <div className ="flex flex-col justify-start">  
            <h1 className ="font-bold mb-2">Ajouter une catégorie parmi les produits</h1>
            <p style = {{color:"#555"}} className ="text-md font-800">Chaque produit doit etre référencé par une catégorie</p>
