...
import { NEW_BC_RESET } from '../../features/constants/blogCategoryConstant';
import {clearErrors} from "../../features/actions/blogCategoryAction"
import {createBc} from "../../features/actions/blogCategoryAction"
export default function NewBlogCategory() {

    const dispatch = useDispatch()
    const alert = useAlert()
    const [show,setShow] = useState(false)
    const {loading,error,success} = useSelector(state => state.bcs)
    ...
    <div className='px-7 py-5 h-full'>
        <form onSubmit = {createProductSubmitHandler} className='h-full p-5 lg:p-10 rounded-md flex flex-row justify-start items-left' style = {{backgroundColor:"#f3f3f3"}}>
          <div className ="flex flex-col justify-start">  
            <h1 className ="font-bold mb-2">Ajouter une catégorie pour les articles</h1>
            <p style = {{color:"#555"}} className ="text-md font-800">Chaque article doit etre référencé par une catégorie</p>
